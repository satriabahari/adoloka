{{-- <div class="bg-white py-16 px-8">
    <div class="container ">
        <!-- Warning Box -->
        <div class="mb-8">
            <h3
                class="text-xl md:text-2xl font-bold mb-2 text-center bg-gradient-to-r from-[#114177] via-[#006A9A] to-[#17A18A] bg-clip-text text-transparent">
                TIDAK PERLU KHAWATIR!
            </h3>

            <p class="text-gray-700 text-center text-sm md:text-base">
                Yang Anda butuhkan sekarang adalah solusi instan yang tepercaya dengan solusi cepat yang dijamin kuat
                tantriya
            </p>
        </div>

        <!-- Features Grid -->
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mx-auto">

            <!-- Feature Card 1 - span 2 column -->
            <div
                class="md:col-span-2 bg-gradient-to-r from-[#114177] via-[#006A9A] to-[#17A18A] rounded-2xl p-8 text-white shadow-xl order-3 md:order-1">
                <h3 class="text-2xl font-bold mb-4">
                    Jaminan Kualitas dan Keahlian Terverifikasi
                </h3>
                <p class="text-blue-100">
                    Terjebak mencari talenta untuk proyek kecil dan menengah
                </p>
            </div>

            <!-- Feature Card 2 -->
            <div
                class="bg-white border-2 border-gray-200 rounded-2xl p-8 shadow-lg hover:shadow-xl transition duration-300 order-1 md:order-2">
                <h3
                    class="text-xl font-bold bg-gradient-to-r from-[#114177] via-[#006A9A] to-[#17A18A] bg-clip-text text-transparent mb-4">
                    Kemudahan Interaksi
                </h3>
                <p class="text-gray-600">
                    Terjebak mencari talenta untuk proyek kecil dan menengah
                </p>
            </div>

            <!-- Feature Card 3 -->
            <div
                class="bg-white border-2 border-gray-200 rounded-2xl p-8 shadow-lg hover:shadow-xl transition duration-300 order-2 md:order-3">
                <h3
                    class="text-xl font-bold bg-gradient-to-r from-[#114177] via-[#006A9A] to-[#17A18A] bg-clip-text text-transparent mb-4">
                    Efisiensi Waktu dan Proses
                </h3>
                <p class="text-gray-600">
                    Platform kami membantu kamu menemukan talenta terbaik dengan cepat
                </p>
            </div>

            <!-- Feature Card 4 - span 2 column -->
            <div
                class="md:col-span-2 bg-gradient-to-r from-[#114177] via-[#006A9A] to-[#17A18A] rounded-2xl p-8 text-white shadow-xl order-4">
                <h3 class="text-2xl font-bold mb-4">
                    Dukungan Profesional dan Transparansi
                </h3>
                <p class="text-blue-100">
                    Kami menjamin proses kerja yang jelas, transparan, dan berkualitas
                </p>
            </div>
        </div>

    </div>
</div> --}}

<div class="bg-white py-16 px-8">
    <div class="container mx-auto">
        <!-- Warning Box -->
        <div class="mb-12 text-center animate-fade-in">
            <h3 class="text-2xl md:text-3xl font-bold mb-4 text-sky-900">
                TIDAK PERLU KHAWATIR!
            </h3>
            <p class="text-gray-600 text-base md:text-lg max-w-3xl mx-auto">
                Yang Anda butuhkan sekarang adalah solusi instan yang tepercaya dengan proses cepat yang dijamin
                berkualitas
            </p>
        </div>

        <!-- Features Grid -->
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mx-auto max-w-7xl">

            <!-- Feature Card 1 - span 2 column -->
            <div class="md:col-span-2 bg-gradient-to-r from-sky-600 via-sky-500 to-sky-600 rounded-2xl p-8 text-white shadow-xl hover:shadow-2xl transition-all duration-300 order-3 md:order-1 group animate-fade-in-up"
                style="animation-delay: 0.1s">
                <div class="flex items-start gap-4">
                    <div
                        class="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform duration-300">
                        <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd"
                                d="M6.267 3.455a3.066 3.066 0 001.745-.723 3.066 3.066 0 013.976 0 3.066 3.066 0 001.745.723 3.066 3.066 0 012.812 2.812c.051.643.304 1.254.723 1.745a3.066 3.066 0 010 3.976 3.066 3.066 0 00-.723 1.745 3.066 3.066 0 01-2.812 2.812 3.066 3.066 0 00-1.745.723 3.066 3.066 0 01-3.976 0 3.066 3.066 0 00-1.745-.723 3.066 3.066 0 01-2.812-2.812 3.066 3.066 0 00-.723-1.745 3.066 3.066 0 010-3.976 3.066 3.066 0 00.723-1.745 3.066 3.066 0 012.812-2.812zm7.44 5.252a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
                                clip-rule="evenodd" />
                        </svg>
                    </div>
                    <div>
                        <h3 class="text-2xl font-bold mb-3">
                            Jaminan Kualitas dan Keahlian Terverifikasi
                        </h3>
                        <p class="text-sky-50">
                            Setiap partner telah melalui proses verifikasi ketat untuk memastikan kualitas dan
                            profesionalitas terbaik, mulai dari keahlian teknis, pengalaman kerja, hingga reputasi
                            pelayanan. Kami memastikan setiap kolaborasi yang terjalin memberikan hasil yang optimal,
                            sehingga kamu dapat bekerja sama dengan mitra yang benar-benar terpercaya dan berkompeten di
                            bidangnya.
                        </p>
                    </div>
                </div>
            </div>

            <!-- Feature Card 2 -->
            <div class="bg-white border-2 border-sky-200 rounded-2xl p-8 shadow-lg hover:shadow-2xl hover:border-sky-400 transition-all duration-300 order-1 md:order-2 group animate-fade-in-up"
                style="animation-delay: 0.2s">
                <div
                    class="w-12 h-12 bg-sky-100 rounded-xl flex items-center justify-center mb-4 group-hover:bg-sky-500 group-hover:scale-110 transition-all duration-300">
                    <svg class="w-6 h-6 text-sky-600 group-hover:text-white transition-colors" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                    </svg>
                </div>
                <h3 class="text-xl font-bold text-sky-900 mb-3 group-hover:text-sky-600 transition-colors">
                    Kemudahan Interaksi
                </h3>
                <p class="text-gray-600">
                    Platform yang user-friendly memudahkan komunikasi antara UMKM dan event organizer
                </p>
            </div>

            <!-- Feature Card 3 -->
            <div class="bg-white border-2 border-sky-200 rounded-2xl p-8 shadow-lg hover:shadow-2xl hover:border-sky-400 transition-all duration-300 order-2 md:order-3 group animate-fade-in-up"
                style="animation-delay: 0.3s">
                <div
                    class="w-12 h-12 bg-sky-100 rounded-xl flex items-center justify-center mb-4 group-hover:bg-sky-500 group-hover:scale-110 transition-all duration-300">
                    <svg class="w-6 h-6 text-sky-600 group-hover:text-white transition-colors" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M13 10V3L4 14h7v7l9-11h-7z" />
                    </svg>
                </div>
                <h3 class="text-xl font-bold text-sky-900 mb-3 group-hover:text-sky-600 transition-colors">
                    Efisiensi Waktu dan Proses
                </h3>
                <p class="text-gray-600">
                    Platform kami membantu kamu menemukan event terbaik dengan cepat dan efisien
                </p>
            </div>

            <!-- Feature Card 4 - span 2 column -->
            <div class="md:col-span-2 bg-gradient-to-r from-sky-600 via-sky-500 to-sky-600 rounded-2xl p-8 text-white shadow-xl hover:shadow-2xl transition-all duration-300 order-4 group animate-fade-in-up"
                style="animation-delay: 0.4s">
                <div class="flex items-start gap-4">
                    <div
                        class="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform duration-300">
                        <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd"
                                d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
                                clip-rule="evenodd" />
                        </svg>
                    </div>
                    <div>
                        <h3 class="text-2xl font-bold mb-3">
                            Dukungan Profesional dan Transparansi
                        </h3>
                        <p class="text-sky-50">
                            Kami menjamin proses kerja yang jelas, transparan, dan berkualitas dengan dukungan tim
                            profesional yang siap
                            membantu di setiap tahap. Seluruh aktivitas dilakukan dengan komunikasi terbuka, pelaporan
                            yang akurat, dan
                            komitmen penuh terhadap integritas, agar setiap langkah kerja dapat dipantau dan
                            menghasilkan kepercayaan yang
                            berkelanjutan antara UMKM dan penyelenggara event.
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

<style>
    @keyframes fadeIn {
        from {
            opacity: 0;
        }

        to {
            opacity: 1;
        }
    }

    @keyframes fadeInUp {
        from {
            opacity: 0;
            transform: translateY(30px);
        }

        to {
            opacity: 1;
            transform: translateY(0);
        }
    }

    .animate-fade-in {
        animation: fadeIn 0.8s ease-out;
    }

    .animate-fade-in-up {
        opacity: 0;
        animation: fadeInUp 0.8s ease-out forwards;
    }
</style>
